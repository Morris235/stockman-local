...
def portfolio_revenue():
    try:
        ...
        daily_ret = df.pct_change()  # 일간 수익률 daily_ret
        ...
        for _ in range(
                30000):  # 포트폴리오 20,000개를 생성하는데 range() 함수와 for in 구문을 사용했다. for in 구문에서 반복횟수를 사용할 일이 없으면 관습적으로 _ 변수에 할당한다.
            weights = np.random.random(len(stocks_name_list))  # 4개의 랜덤 숫자로 구성된 배열을 생성한다.
            ...
            sharpe_ratio.append(returns / risk)  # *포트폴리오의 수익률을 리스크로 나눈 값을 샤프 지수 리스트에 추가한다.* 제대로된 계산을 하기 위해선 공부를 좀 더 해야함

        portfolio = {'Returns': port_ret, 'Risk': port_risk, 'Sharpe': sharpe_ratio}
        for i, s in enumerate(
                stocks_name_list):  # i값은 0,1,2,3 순으로 변한다. 이 때 s값은 '삼성전자', 'SK하이닉스', '현대자동차', 'NAVER'순으로 변한다.
        ...
        df = df[['Returns', 'Risk', 'Sharpe'] + [s for s in stocks_name_list]]  # 샤프 지수 칼럼을 데이터프레임에 추가한다.
        ...
        for s in stocks_name_list:
            ...
            amount = int(stock_investment / df_price[s].values[0])
            # print(f"{df_price[s].name} 매입가: {df_price[s].values[0]} 할당된 투자금 : {stock_investment} 매입수: {amount}")
            buy = df_price[s].values[0] * amount
            sale = df_price[s].values[-1] * amount
            revenue = sale - buy

            print(
                f"[{df_price[s].name}] 할당된 투자금: {stock_investment}, 매수종가:{df_price[s].values[0]}, 매도종가: {df_price[s].values[-1]}, 매입개수:{amount}, 매수 : {buy}, 매도 : {sale}, "
                f"수익 : {revenue}")

            total_investment.append(buy)

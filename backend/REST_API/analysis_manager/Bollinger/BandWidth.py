...
df['bandwidth'] = (df['upper'] - df['lower']) / df['MA20'] * 100  # (상단 볼린저 밴드 - 하단 볼린저 밴드) / 중간 볼린저 밴드 * 100 을 구해 bandwidth(밴드폭) 칼럼을 생성한다.
...
plt.plot(df.index, df['bandwidth'], color='m', label=' BandWidth')  # x좌표 df.index에 해당하는 bandwidth값을 y좌표로 설정해 자홍색(magenta) 실선으로 표시한다.

...
class StateReader:
    ...
    def concat_income_state(self, code: str, year_dir: str):
        try:
            # 연결 손익계산서
            coverage_link_dir = f'business_report/{year_dir}/year/{self.income_txt[3]}'
            normal_link_dir = f'business_report/{year_dir}/year/{self.income_txt[1]}'
            coverage_link_df = self.convert_dataframe(coverage_link_dir, code)
            normal_link_df = self.convert_dataframe(normal_link_dir, code)

            # 별도 손익계산서
            coverage_dir = f'business_report/{year_dir}/year/{self.income_txt[2]}'
            normal_dir = f'business_report/{year_dir}/year/{self.income_txt[0]}'
    ...
    def concat_fin_state(self, code: str, year_dir: str):
        try:
            ...
            # 연결 재무상태 보고서
            link_dir = f'business_report/{year_dir}/year/{self.fin_txt[1]}'
            fin_state_link_df = self.convert_dataframe(link_dir, code)

            # 별도 재무상태 보고서
            normal_dir = f'business_report/{year_dir}/year/{self.fin_txt[0]}'
